...
| `config-print` | what this machine's configuration actually resolves to: the provider, the model for each role, and which ecosystem the suggestions will target with the reason it was chosen. No API call |
| `provider-print` | one line: the resolved provider id, or why there isn't one. A machine contract — `provider-check` spawns it as a child and parses this output, so nothing may be added to it. Use `config-print` to read it yourself |
