...
public enum DaemonControl {
    ...
    public static let daemonExecutableName = "DeskMateDaemon"
    ...
    public static func daemonExecutableURL() -> URL? {
        if let override = ProcessInfo.processInfo.environment["DESKMATE_DAEMON_PATH"] {
    ...
    public enum StartError: LocalizedError {
        ...
        public var errorDescription: String? {
            switch self {
            case .binaryNotFound:
                return "Couldn't find the DeskMateDaemon binary next to the dashboard. "
                     + "Run `swift build` so both land in the same directory, or set DESKMATE_DAEMON_PATH."
