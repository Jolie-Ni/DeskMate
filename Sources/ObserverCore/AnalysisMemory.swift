...
/// A label previously computed for a session, kept so a re-run doesn't pay
/// Haiku again for work it already did. Session ids are a stable hash of
/// (start, bucket), so they survive re-clustering unchanged.
