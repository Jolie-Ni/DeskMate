...
The shared half of Local Observer. Employees keep everything locally; when
...
export OBSERVER_DB_URL=postgresql://postgres:postgres@localhost:55432/observer_dev
...
Point the desktop app at it with `OBSERVER_HUB_URL=http://localhost:8788`, and
`OBSERVER_STORAGE_DIR=/some/scratch` if you would rather not enrol the database
you actually use.
...
it refuses to start unless `OBSERVER_DB_URL` names a database ending in
`_test`.
...
Links do not expire (see Known gaps). `OBSERVER_SECRET` signs them, which is
...
.venv/bin/python envrun.py .env.production.local \
  OBSERVER_DB_URL=DATABASE_URL,OBSERVER_SECRET=OBSERVER_SECRET \
...
Both refuse to run if `OBSERVER_SECRET` is unset or still the dev default — a
...
```sh
vercel logs local-observer-hub.vercel.app --json > /tmp/logs.json   # ^C after a few seconds
...
```sh
cd server
vercel link --yes --project local-observer-hub
vercel deploy --prod
...
| `OBSERVER_DB_URL` | Overrides `DATABASE_URL`. Used for local development and for pointing `manage.py` at production. |
| `OBSERVER_SECRET` | Signs admin sign-in links. **Set this before creating any org** — the default is a public dev value, and anyone who knows it can forge a sign-in link. |
...
```sh
python3 -c "import secrets;print(secrets.token_urlsafe(32))" | vercel env add OBSERVER_SECRET production
```
...
.venv/bin/python envrun.py ../.env OBSERVER_DB_URL=DATABASE_URL_UNPOOLED \
  .venv/bin/python manage.py init

# everything else can use the pooled one
.venv/bin/python envrun.py ../.env OBSERVER_DB_URL=DATABASE_URL \
...
`Config.hubURL` is `https://local-observer-hub.vercel.app` — the host that
actually resolves. It used to be an aspirational `hub.localobserver.app`, on a
domain nobody had registered, which fails as "a server with this host name
can't be found" and reads like a network problem rather than a wrong constant.

If you register a real domain later, add it in the Vercel project and change
that one line. `OBSERVER_HUB_URL` overrides it per run.
